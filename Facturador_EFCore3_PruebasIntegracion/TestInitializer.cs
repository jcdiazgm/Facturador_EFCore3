...
namespace Facturador_EFCore3_PruebasIntegracion
{
    [TestClass]
    public class TestInitializer
    {
        ...
        public static FacturadorDBContext ObtenerDataContext(bool beginTransaction = true)
        {
            var options = new DbContextOptionsBuilder<FacturadorDBContext>()
                .UseSqlServer($"Data Source=(localdb)\\mssqllocaldb;Initial Catalog={_dbName};Integrated Security=True")
                .Options;
            var context = new FacturadorDBContext(options);
            if (beginTransaction)
            {
